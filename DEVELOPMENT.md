...
- 普通模式下可能出现 `GameState` 仍确认某个 PlayerState 在 `LiveSurvivors`，但 `PlayerArray.PawnPrivate` 暂时为空的情况。Reader 会在这种情况下使用最近 8 秒内观察到的非观战 survivor Character 作为保守回补，计入 `pa_cached` 并在 `player_array_debug[].reason` 写入 `live_survivor_cached_actor`；一旦 PlayerState 不再位于 `LiveSurvivors`，缓存不会继续参与绘制，避免普通击杀进观战或感染转猎人后继续显示旧目标。
- `game_state_membership` 会附带 `mode_label` 和 `main_phase_label`，并把阵营数组修正后的 `role/form` 传入最终 `TargetSnapshot`，让颜色和猎人过滤优先跟随 GameState 阵营，而不是只依赖类名。
...
- 调试页“数据记录”会每秒写入 `logs/runtime_debug_*.jsonl`，并提供“打开日志”按钮用于打开当前运行日志目录。日志用于分析候选目标、过滤统计、投影原因、名称候选、角色形态、边缘提示和最终绘制结果。日志包含 `performance`、`projection_reasons`、`edge_reasons`、`player_array_debug`、`level_actor_debug`、`emitted_targets`、`config_changes` 和 `reader_context`：先看采样/绘制耗时是否异常，再看目标是否进入 PlayerArray、是否被过滤、位置来源和投影失败原因；如果 `drawn=0` 但 `radar_targets` 仍有目标，优先看 `config.esp_enabled` 和 `config_changes`，热键触发会标记为 `hotkey:esp`。`reader_context` 会记录 `GameState`、关卡和本地 Pawn 的类名/对象名，用于区分不同对局模式并决定是否需要模式化的 `SpectatePawn` 链接策略；开启数据记录时还会额外记录 `game_state_fields`，包含 `GameMode` 原始值、`MainGamePhase`、`CurrentGamePhase`、计时器、`MapIndex`、`LiveSurvivors_PlayerState` 和 `HuntersPlayerState` 等 SDK 偏移字段。`tools/analyze_runtime_debug.py` 会把这些字段整理为“对局时间线”，显示模式、阶段、LiveSurvivors/Hunters 数量、PlayerArray 统计、cached 和 suppressed 变化点。`context_event` 会记录 world/GameState/Level 上下文变化，变化时 Reader 会清理跨局身份、名称、pawn 生命周期和位置缓存。`spectate_link` 会记录链接角色的类名、角色/形态、`LastMyPlayerState`、`APawn::PlayerState`、Controller、Dead、坐标来源、root 和 mesh，并附带 `SpectateTarget`、`MyMainBody`、`CanBackBody` 等 `BP_SpectatePawn_cLeon_C` 命名字段。`player_array_debug`、`level_actor_debug` 和 `emitted_targets` 在数据记录帧会记录 `character_flags`，包含 `IsHunter`、`IsLiveSelf`、`BodyVisibility`、`HideBlock`、`CurrentNamePlateVisibility`、物理混合 bit 和 `PhysicsBlendWeight`；这些字段只对真实 Character 读取，避免 Spectate/空对象产生随机值。正常游玩时仍建议关闭“数据记录”。若出现“候选/绘制/雷达数量相同但游戏内感觉少人”，重点看 `dead_or_spectator`、`spectate_link`、`pa_linked`、`pa_cached`、`pa_suppressed`、`game_state_membership`、`role/stable_role/filter_role` 是否解释了被过滤对象。
