...
def _prepare_master_icon(source):
    crop = _content_rect(source).adjusted(-18, -18, 18, 18).intersected(source.rect())
    side = max(crop.width(), crop.height())
    square = QRect(
        crop.center().x() - side // 2,
        crop.center().y() - side // 2,
        side,
        side,
    ...
    target = QRect(26, 26, 972, 972)
    painter.drawImage(target, cropped)
    painter.end()
    return _remove_white_background(icon)
